  ...
  session_start(); 
   if(!isset($_SESSION['username'])){
		...
		if (!$con)
		 ...
		 {
			if($result = mysql_query("SELECT * FROM race_user WHERE UserName ='$countname'"))
	       {
		      
			echo  "<form  action=\"user_competition_exit.php\" method=\"get\" >";
			 while($row = mysql_fetch_array($result))
			  { 
		
					$Id=$row['RaceName'];
					if($result2 = mysql_query("SELECT * FROM organization_race WHERE personID ='$Id'"))
					{		
					 ...
					 if($row2 = mysql_fetch_array($result2))
					 {
						...
						$userPlace=$row2["UserPlace"];
						$userTime=$row2["UserTime"];
						$userStart=$row2["UserStart"];
						$userEnd=$row2["UserEnd"];		
						//echo $userMycomment;
